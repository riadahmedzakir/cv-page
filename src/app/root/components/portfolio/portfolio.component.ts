...
export class PortfolioComponent implements OnInit, AfterViewInit {
  ...
  animateDetailsBox(): void {
    ...
    profileImage.classList.add('animate__animated', 'animate__fadeInLeft', 'animate__slow');

    const profileDetails = document.querySelector('.profile-details');
    profileDetails.classList.add('animate__animated', 'animate__fadeInRight', 'animate__slow');

    const socialMedia = document.querySelector('.profile-social-media');
    socialMedia.classList.add('animate__animated', 'animate__fadeInUp', 'animate__slow');
    ...
    setTimeout(() => {
      const profileInnerContainer = document.querySelector('.profile-inner-container');
      profileInnerContainer['style'].boxShadow = '0 3px 1px -2px rgb(0 0 0 / 20%), 0 2px 2px 0 rgb(0 0 0 / 14%), 0 1px 5px 0 rgb(0 0 0 / 12%)';
    }, 1000);
  }
  ...
  animateExperienceTimeline(): void {
    const observer = new IntersectionObserver((entries) => {
      ...
      if (entries[0]['intersectionRatio'] >= 0.1) {
        const activityContents = document.getElementById('experience').querySelectorAll('.desc-box-text');
        _forEach(activityContents, content => {
          content.classList.add('animate__animated', 'animate__fadeInRight', 'animate__slower');
        });
      }
    }, { threshold: [0.1, 0.5] });
  ...
  animateProjectTimeline(): void {
    const observer = new IntersectionObserver((entries) => {
      ...
      if (entries[0]['intersectionRatio'] >= 0.1) {
        const activityContents = document.getElementById('project').querySelectorAll('.desc-box-text');
        _forEach(activityContents, content => {
          content.classList.add('animate__animated', 'animate__fadeInRight', 'animate__slower');
        });
      }
  ...
  animateToolsCards(): void {
    const observer = new IntersectionObserver((entries) => {
      ...
      if (entries[0]['intersectionRatio'] >= 0.1) {
        const activityContents = document.getElementById('tools').querySelectorAll('.tool-list-chip');
        _forEach(activityContents, content => {
          content.classList.add('animate__animated', 'animate__fadeIn', 'animate__slower');
        });
      }
    }, { threshold: [0.1, 0.5] });
